...
This is a short, practical guide to run the `dev-box` image as a Kubernetes Pod. Keep secrets out of plain env-vars in production.

## Prerequisites
- Build the image: `docker build -t dev-box:latest -f src/Dockerfile .`
- Make the image available to your cluster:
  - kind: `kind load docker-image dev-box:latest`
  - minikube: `minikube image load dev-box:latest`
  - Docker Desktop: the local daemon is usually shared
  - Or push to a registry and use `your-registry/dev-box:tag` in manifests

## Quick test (not recommended for production)
- Run via `kubectl run` with an env var password:

  `kubectl run dev-box --image=dev-box:latest --restart=Never --port=3389 --image-pull-policy=IfNotPresent --env="USER_PASSWORD=s3cr3t"`
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: dev-box
        image: gcr.io/PROJECT_ID/dev-box:latest # replace PROJECT_ID
        imagePullPolicy: IfNotPresent
        ports:
        - containerPort: 3389
        env:
